...
func GetCpuTicks(cpuid ...int) (ticks cpuTicks, err error) {
	...
	if len(fields) >= 6 {
		ticks.iowait, _ = strconv.ParseUint(fields[6], 10, 64)
	}
	if len(fields) >= 7 {
		ticks.irq, _ = strconv.ParseUint(fields[7], 10, 64)
	}
	if len(fields) >= 8 {
		ticks.softirq, _ = strconv.ParseUint(fields[8], 10, 64)
	}
	if len(fields) >= 9 {
		ticks.steal, _ = strconv.ParseUint(fields[9], 10, 64)
	}
	if len(fields) >= 10 {
		ticks.guest, _ = strconv.ParseUint(fields[10], 10, 64)
	}
	if len(fields) >= 11 {
		ticks.guestNice, _ = strconv.ParseUint(fields[11], 10, 64)
	}
